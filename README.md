...
| SMS nhận/gửi | ✅ | ✅ |
| Call metadata + điều khiển (answer/reject/hold/DTMF) | ✅ | ✅ (metadata, không điều khiển audio) |
...
- **Bluetooth HFP SCO** chỉ dùng cho **audio cuộc gọi** (Android 10+ chặn capture call audio qua API công khai; HFP là con đường duy nhất đã được chứng minh). Fallback: **Opus-over-WebSocket** (~100–150ms) khi HFP không khả dụng.
- **E2E encryption** bắt buộc toàn hệ thống: XChaCha20-Poly1305 payload, X25519 + HKDF key exchange, ghép cặp qua **QR code**. Audio có **dual-layer** (BT SSP + app-level E2E).
