...
Cửa sổ nổi nhỏ trên Mac cho cuộc gọi đến và cuộc gọi đang diễn ra (CALL-01…03, AUDIO-03). Trông như
thông báo cuộc gọi Continuity của Apple: tấm kính ở góc trên bên phải, không lấy focus của app người
dùng đang làm việc.

**Lệch có chủ đích so với HIG:** HIG muốn panel ẩn khi app không active; cuộc gọi đến thì phải hiện
đúng lúc đó. Panel vì thế là `NSPanel` non-activating nổi trên mọi Space. Mỗi cuộc gọi còn có một
thông báo liên lạc (`INStartCallIntent`): mức passive khi panel đang hiện (chỉ vào Trung tâm thông
báo, không hiện chồng banner), mức time-sensitive khi Tập trung bật và panel không hiện.
...
Tắt tiếng, Giữ máy, Bàn phím chỉ có qua Bluetooth HFP. Khi Mac chưa nối, **ẩn** ba nút này và thay
bằng dòng "Nối Bluetooth với điện thoại để giữ máy, bấm số, tắt tiếng" (CALL-03 E2). Lỗi hiện ngay
trong panel: "Điện thoại không thực hiện được thao tác này", "Mất kết nối với điện thoại".
...
- Tập trung đang bật → không hiện panel, không đổ chuông; thông báo time-sensitive để hệ thống quyết
  ...
  (thiếu quyền) → hiện panel, không đổ chuông. Lệch so với CALL-01 E4.
...
- Nên giữ Từ chối bên trái, Trả lời bên phải ở mọi trạng thái; khác cả màu, biểu tượng
  (`phone.down.fill` / `phone.fill`) và vị trí.
