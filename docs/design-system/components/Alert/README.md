...
| Delete all HandLive data, remove the device from the server (SET-02 field 28) | Same as above: "Delete All HandLive Data?" with "Delete All", or "Remove This Device from the Server?" with "Remove from Server" | Same as above, with the same titles and buttons |
