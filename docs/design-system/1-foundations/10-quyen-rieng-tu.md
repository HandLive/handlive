...
| Local network (macOS 15+) | `Onboarding`, the first Bonjour browse | `NSLocalNetworkUsageDescription`: "HandLive looks for your Android phone on your Wi-Fi network to connect to it directly, without going through the internet." |
...
- Exactly one "Continue" button; choosing it opens the system dialog. No "Skip", "Cancel", "Later", or
...
- Say it in one sentence in `Onboarding`: "Your data is end-to-end encrypted and travels only between
