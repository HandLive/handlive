...
| 28 | Destructive action confirmation | enum{Remove from Server\| Delete All\| Cancel} | Input | — | Titled with the action's name without the ellipsis, "Remove Device from Server" or "Delete All HandLive Data" (Mac, iPhone, iPad and Android alike), with the field 29 warning as its message. The confirm button names the action and sits next to "Cancel": "Remove from Server" for field 26, "Delete All" for field 27; it is warning-colored |
...
| 11 | Limits on iPhone/iPad | string | Output | Fixed text | iOS/iPadOS. Title "How HandLive Works on iPhone and iPad", text "The clipboard syncs while HandLive is open on this device: tap the Paste button to send, with no paste permission prompt. iPhone and iPad can't take calls. When HandLive is closed, SMS messages and incoming calls appear as notifications." |
...
| 2 | System | M-APP / I-APP, OS (Keychain) | No `setup.started_at` (fresh install) → delete Keychain items left over from a previous install (service `app.handlive.keys` — iOS/macOS keep the Keychain after the app is removed), create `ik_sig`, `ik_dh` (CryptoKit) and a 32-byte `db_key` (`SecRandomCopyBytes`) and store them in the Keychain (API 1); create `handlive.sqlite` with the 0.9.3 schema (iOS/iPadOS: in the app's own container, not in the App Group container: I-NSE never opens it, and iOS terminates a suspended app that holds an SQLite lock on a file in a shared container — 0.9.3); write `setup.started_at`.<br>`setup.started_at` already set → load the existing keys.<br>Compute `device_id` (0.2).<br>Show the welcome screen (field 1). | Error → E1. |
