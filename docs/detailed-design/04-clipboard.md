...
| 10 | Content too large message | string | Output | — | "Content is too large to send (up to 1 MB of text)" — toast (both the automatic and the manual path), no system notification |
| 11 | Manual send result | string | Output | — | Toast "Sent to \<name>", "Not connected — will send if reconnected within 2 minutes" or "The clipboard is empty or doesn't contain text"<br>The other device refuses (an error other than `FEATURE_DISABLED`, `CLIP_TOO_LARGE`): "Couldn't update the clipboard on \<name>". Skipped because it matches what was just received (QC4, E9): "Not sent — this just came from \<name>." |
