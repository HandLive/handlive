...
> Tham chiếu chung: [`00-common-specs.md`](00-common-specs.md) — thành phần (0.1), định danh (0.2), khung tin (0.5), khóa và ghép nối (0.6.1–0.6.2), mã lỗi (0.8), lược đồ (0.9).
...
| Mô tả | Thiết lập quan hệ tin cậy giữa điện thoại Android và một Mac/iPhone/iPad: trao đổi khóa định danh, sinh khóa gốc của cặp `PRK`, ghim chứng chỉ TLS của điện thoại, lưu bản chứng thực có chữ ký của cả hai bên. Mac/iOS hiển thị QR, Android quét. Khi không quét được QR thì dùng PIN 6 số (chỉ trong LAN). Từ P2, nếu hai thiết bị không thấy nhau trong LAN, bước trao đổi đi qua điểm hẹn trên relay (chỉ với QR). |
| Tác nhân | Chính: Người dùng (sở hữu cả hai thiết bị). Hệ thống: A-UI, A-SVC, M-APP hoặc I-APP, R-API và R-KV (điểm hẹn, đăng ký cặp). |
| Điều kiện trước | 1. Android đã hoàn tất SET-01 (có quyền camera để quét QR) và A-SVC đang chạy. 2. Mac/iOS đã hoàn tất SET-03 (quyền mạng cục bộ). 3. Hai thiết bị cùng LAN; hoặc (từ P2) cả hai có Internet và `relay.enabled = true`. 4. Mac/iOS chưa có cặp hiệu lực nào. 5. Android có ít hơn 8 cặp hiệu lực. |
| Điều kiện sau | **Thành công:** hai bên có bản ghi `paired_device` cùng `pair_id`; `PRK` nằm trong kho khóa; Mac/iOS ghim `peer_tls_sha256`; cặp được đăng ký lên relay nếu relay bật (hoặc đánh dấu chờ đăng ký); CONN-01 tự chạy. **Thất bại:** không bên nào lưu gì; `pairing_secret` hoặc PIN bị hủy khỏi bộ nhớ. |
| Ngoại lệ | E1 — QR không phải của HandLive hoặc sai định dạng (`QR_INVALID`). E2 — QR đã hết hạn vì Mac/iOS đã làm mới (`PAIRING_CLOSED`). E3 — Không tìm thấy nhau trong 20 s và relay không khả dụng. E4 — HMAC, chữ ký hoặc ràng buộc TLS sai, có thể đang bị tấn công xen giữa (`AUTH_FAILED`). E5 — Người dùng bấm Hủy trên Android. E6 — Android đã đủ 8 cặp. E7 — PIN sai (`PIN_INVALID`); quá 3 lần thì Mac/iOS sinh PIN mới. E8 — Đăng ký cặp lên relay lỗi: cặp vẫn dùng được trong LAN, `relay_registered = 0`, thử lại nền. E9 — Quyền camera bị từ chối: gợi ý chuyển sang PIN. |
| Yêu cầu đặc biệt | **Bảo mật:** `pairing_secret` 256 bit, chỉ sống 120 s, không bao giờ rời thiết bị ngoài QR, không ghi log; so sánh HMAC hằng thời gian; relay không nhìn thấy `pairing_secret` và không được dùng cho PIN. PIN là đường dự phòng: kẻ tấn công chủ động nằm giữa đúng lúc ghép có thể dò PIN ngoại tuyến — giảm thiểu bằng Argon2id (t=3, m=64 MiB, p=4), giới hạn 3 lần và chỉ cho phép trong LAN. **Hiệu năng:** từ lúc quét tới "Đã ghép nối" ≤ 5 s trong LAN, ≤ 8 s qua relay. **Khả dụng:** QR đủ tương phản ở cả giao diện sáng và tối; hướng dẫn đọc được bằng VoiceOver/TalkBack; nhận diện QR chạy hoàn toàn trên máy (ML Kit bản đóng gói). |
...
| 2 | Hệ thống | M-APP / I-APP | Nạp hoặc tạo `ik_dh`, `ik_sig`; sinh `pairing_secret` 32 byte (`SecRandomCopyBytes`). Nếu relay bật: sinh `rv_id` 16 byte, xác thực relay (0.6.4), gửi `rv_join`. Dựng URI QR, vẽ QR (`CIFilter.qrCodeGenerator`). Bắt đầu duyệt mDNS `_handlive._tcp`. Hẹn 120 s: hủy secret cũ, quay lại bước 2. | Relay lỗi → QR không có `rv`, chỉ ghép trong LAN. |
...
| 11 | Hệ thống | M-APP ↔ A-SVC, R-API | Client tính `PRK`, sinh `pair_id`, `created_at`, bản chứng thực, ký `sig_c`, gửi `pair/confirm`. Android kiểm `mac`, `prk_check`, `device_id` và `sig_c`; ký `sig_s`; lưu `paired_device`; trả `pair/done`. Client kiểm `sig_s`, `prk_check`; lưu `paired_device`, `PRK` vào Keychain, ghim chứng chỉ. Cả hai hủy secret; Android đóng `/v1/pair`, bỏ TXT `pr`, thêm hint của cặp mới vào TXT `h`. Nếu relay bật, mỗi bên gọi `POST /v1/pairs` (idempotent). | Relay lỗi → E8, thử lại nền khi có mạng. |
...
- `K_pa` = HKDF-SHA256(ikm = `pairing_secret` (hoặc `K_pin`), salt = `nonce_c` ‖ `nonce_s`, info = `"handlive/v1/pair-auth"`, L = 32).
- `T_offer` = `"HL1|offer|"` ‖ các trường theo thứ tự cố định: `device_id`C(16) ‖ `nonce_c`(32) ‖ `ik_sig_pub`C(32) ‖ `ik_dh_pub`C(32) ‖ str(`name`C) ‖ `device_id`S(16) ‖ `nonce_s`(32) ‖ `ik_sig_pub`S(32) ‖ `ik_dh_pub`S(32) ‖ `tls_sha256`(32) ‖ str(`name`S), với str(x) = uint16 BE độ dài ‖ UTF-8.
...
- **Ví dụ:** `handlive://pair?v=1&pk=q83vEjRWeJC7zN3u_wARIjNEVWZ3iJmqu8zd7v8AESI&ps=AAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8&d=MacBook%20c%E1%BB%A7a%20Lan&rv=Eh8kKS4zOD1CR0xRVltgZQ`
- **Logic nghiệp vụ:**
  1. Chuỗi URI ≤ 300 ký tự để QR ở mức phiên bản ≤ 10, quét tốt trên màn hình Retina.
  2. Android bỏ qua tham số không biết (tương thích tiến); chỉ từ chối khi thiếu tham số bắt buộc, sai độ dài hoặc `v` khác `1`.
  3. Mỗi lần làm mới QR tạo `pairing_secret` và `rv_id` mới; secret cũ bị xóa khỏi bộ nhớ ngay.

#### API 2 — `WS pair/hello`

- **URL:** `wss://{android_host}:{port}/v1/pair` (LAN) hoặc envelope trong `rv_msg` qua `wss://{RELAY_HOST}/v1/relay`
...
- **Logic nghiệp vụ:**
  ...
  2. `mode = qr`: `ik_dh_pub` phải trùng `pk` đã quét, sai → `AUTH_FAILED`. `mode = pin`: chỉ nhận khi cửa sổ mở ở chế độ PIN và kết nối đến từ LAN (không qua relay).
  3. Kiểm `device_id` = UUIDv8(SHA-256(`ik_sig_pub`)); sai → `AUTH_FAILED`.
  4. Một cửa sổ ghép nối chỉ phục vụ một client: kết nối thứ hai trong cùng cửa sổ → `PAIRING_CLOSED`.
...
- **Logic nghiệp vụ:**
  ...
  2. `tls_sha256` được bảo vệ bởi `mac`, nên kể cả qua relay (không có kênh TLS trực tiếp) client vẫn ghim đúng chứng chỉ thật của điện thoại.
  3. Client kiểm theo thứ tự: `mac` → `device_id` khớp `ik_sig_pub` → (LAN) `tls_sha256` trùng chứng chỉ của kết nối hiện tại. Bất kỳ bước nào sai → `AUTH_FAILED`, không tiết lộ bước nào sai.
...
- **Logic nghiệp vụ:**
  1. Android kiểm `mac` trước, rồi tự tính `PRK` (0.6.2) và so `prk_check` — phát hiện sớm mọi sai lệch cài đặt giữa hai nền tảng.
  2. Dựng lại `attestation` từ dữ liệu đã có, kiểm `sig` bằng `ik_sig_pub` của client.
  3. `|created_at − giờ Android| > 10 phút` → vẫn nhận (đồng hồ có thể lệch) nhưng dùng nguyên giá trị client gửi trong `attestation`.
  4. Lưu cặp trong một giao dịch: nếu đã có bản ghi cùng `peer_device_id` (client ghép lại) thì xóa bản cũ rồi chèn bản mới.
...
- **Logic nghiệp vụ:**
  1. Client kiểm `mac`, `prk_check`, `sig`; sai → xóa mọi dữ liệu tạm, báo E4 (Android đã lưu cặp sẽ tự dọn khi CONN-01 thất bại với `PAIR_UNKNOWN` phía client — xem PAIR-03 bước dọn dẹp).
  2. Đúng → client lưu `paired_device`, lưu `PRK` vào Keychain (account = `pair_id`), rồi mới báo thành công cho người dùng.
...
- **Response:** N/A.
- **Ví dụ:** `{"op":"error","data":{"code":"PIN_INVALID","message":"Mã PIN không đúng","attempts_left":2}}`
- **Logic nghiệp vụ:** Không tiết lộ chi tiết bước kiểm nào sai với `AUTH_FAILED`; ghi log cục bộ chỉ gồm mã lỗi.
...
- **Logic nghiệp vụ:**
  ...
  3. Relay chuyển nguyên `env` sang thành viên còn lại, không đọc nội dung; chỉ chấp nhận `env.type = "pair"`.
  4. Envelope `pair/hello` có `mode = "pin"` qua điểm hẹn bị relay chặn và Android cũng từ chối (PIN chỉ trong LAN).
...
- **Logic nghiệp vụ:**
  ...
  3. Phân tích `attestation`, đối chiếu `pair_id`, `device_a`, `device_b`, `created_at` và hai khóa công khai; kiểm `sig_a`, `sig_b`.
...
| Mô tả | Hiển thị các thiết bị đã ghép nối cùng trạng thái kết nối trực tiếp, kênh đang dùng (LAN, Internet, USB), lần kết nối cuối, phiên bản, tính năng đang hiệu lực và quyền còn thiếu trên điện thoại. Android thấy danh sách tối đa 8 client; Mac/iOS thấy một điện thoại. Từ màn hình này người dùng đi tới PAIR-01 (thêm), PAIR-03 (hủy) hoặc SET-02 (tùy chọn). Đồng thời đối chiếu với relay để phát hiện cặp đã bị thu hồi từ thiết bị khác. |
...
| Ngoại lệ | E1 — Chưa có cặp nào: hiển thị trạng thái trống và nút "Thêm thiết bị". E2 — Relay không truy cập được: chỉ hiển thị dữ liệu cục bộ, không báo lỗi chặn. E3 — Relay báo một cặp đã thu hồi: dọn cặp đó (PAIR-03, luồng B) và báo "Thiết bị <tên> đã được hủy ghép nối từ thiết bị khác". E4 — Lỗi đọc cơ sở dữ liệu: báo lỗi và cho thử lại. |
| Yêu cầu đặc biệt | Trạng thái cập nhật trong ≤ 1 s sau khi kết nối thay đổi (theo dõi luồng trạng thái, không hỏi vòng). Không hiển thị khóa, `pair_id` hay dấu vân tay đầy đủ; "Mã an toàn" chỉ là 8 ký tự hex đầu SHA-256(`attestation`) để người dùng đối chiếu giữa hai thiết bị nếu muốn. Đọc được bằng VoiceOver/TalkBack. |
...
Capability hiển thị ở trường 7–9 lấy từ `capability/hello` và `capability/update` đã nhận trong CONN-01 và SET-02, không phát sinh lời gọi mới.
...
- **Logic nghiệp vụ:**
  ...
  3. Cặp có trên thiết bị nhưng **không có** trên relay (khác với có `revoked_at`) không bị coi là thu hồi. Nếu `relay_registered = 1` thì đối phương đã tự gỡ khỏi relay ("Xóa thiết bị khỏi máy chủ", SET-02) → đặt `relay_registered = 0`, giữ cặp và chỉ dùng LAN/USB. Mọi cặp `relay_registered = 0` được thử lại `POST /v1/pairs` (PAIR-01 API 8) khi có mạng; nhận 404 `DEVICE_NOT_FOUND` (đối phương chưa đăng ký lại) thì thử lại sau 24 h.
...
| Mô tả | Chấm dứt quan hệ tin cậy của một cặp. **Luồng A (cả hai đang kết nối):** bên khởi tạo gửi `pair/revoke`, bên kia xác nhận, cả hai xóa khóa và dữ liệu. **Luồng B (đối phương không kết nối được, ví dụ máy bị mất):** bên khởi tạo xóa cục bộ và thu hồi trên relay; đối phương tự dọn khi quay lại mạng (relay báo `pair_revoked`, hoặc Android trả `PAIR_UNKNOWN` trong LAN). Dữ liệu đã đồng bộ (SMS, nhật ký cuộc gọi) trên Mac/iOS bị xóa cùng cặp. |
...
| Điều kiện sau | **Bên khởi tạo:** không còn `PRK`, bản ghi cặp và dữ liệu đồng bộ của cặp; Android bỏ hint của cặp khỏi TXT `h`; relay đánh dấu `revoked_at` (ngay hoặc khi có mạng). **Đối phương:** dọn giống hệt khi nhận được tín hiệu thu hồi. Phiên `/v1/ctl` của cặp bị đóng. |
| Ngoại lệ | E1 — Người dùng hủy ở hộp thoại xác nhận. E2 — Đối phương không phản hồi `ack` trong 10 s → chuyển luồng B. E3 — Relay không truy cập được → giữ bản ghi "bia mộ" (`revoked_at` đặt, dữ liệu và khóa đã xóa), thử thu hồi lại nền mỗi lần có mạng. E4 — Cặp đã bị thu hồi từ trước trên relay → coi là thành công. |
| Yêu cầu đặc biệt | Bảo mật: xóa `PRK` khỏi Keychain/Keystore trước khi báo thành công; không cho hoàn tác (muốn dùng lại phải ghép nối mới). Hộp thoại xác nhận nêu rõ dữ liệu sẽ bị xóa. Hiệu năng: luồng A hoàn tất ≤ 2 s. Thu hồi từ xa có hiệu lực trên relay ngay khi `POST` thành công: relay ngừng chuyển tiếp và push cho cặp đó. |
...
- **Response (`ack.data`):** `{}` khi `ok = true`. Lỗi: `BAD_REQUEST` nếu `pair_id` không khớp phiên.
- **Ví dụ:**
...
- **Logic nghiệp vụ:** Bên nhận `reason = revoked` dọn cặp nếu chưa dọn (phòng khi mất `pair/revoke`).
...
- **Logic nghiệp vụ:**
  ...
  4. Từ thời điểm này relay từ chối mọi khung `to`/`from` giữa hai thiết bị của cặp (`relay.error NOT_PAIRED`) và từ chối `POST /v1/push` cho cặp.
...
- **Response:** N/A.
- **Ví dụ:** `{"op":"pair_revoked","pair_id":"3f2b1c4d-5e6f-4a7b-8c9d-0e1f2a3b4c5d","by":"8c7d6e5f-4a3b-8c2d-9e1f-0a1b2c3d4e5f"}`
- **Logic nghiệp vụ:**
  1. Thiết bị nhận dọn cặp như bước 7 và hiển thị trường 5; nhận lặp lại thì bỏ qua.
  2. Ngay sau khi một thiết bị kết nối relay, relay gửi `pair_revoked` cho mọi cặp đã thu hồi trong 30 ngày mà thiết bị là thành viên, để thiết bị offline lúc bị thu hồi vẫn tự dọn.
