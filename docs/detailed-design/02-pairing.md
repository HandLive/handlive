...
| Exceptions | E1 — The QR code is not a HandLive code or is malformed (`QR_INVALID`).<br>E2 — The QR code has expired because Mac/iOS has refreshed it (`PAIRING_CLOSED`).<br>E3 — The devices don't find each other within 20 s and the relay is unavailable.<br>E4 — Wrong HMAC, signature or TLS binding; possibly a man-in-the-middle attack (`AUTH_FAILED`).<br>E5 — The user taps Cancel on Android.<br>E6 — Android already has 8 pairs.<br>E7 — Wrong PIN (`PIN_INVALID`); after 3 failed attempts Mac/iOS generates a new PIN.<br>E8 — Registering the pair with the relay fails: the pair still works on the LAN, `relay_registered = 0`, retry in the background.<br>E9 — Camera permission denied: suggest switching to the PIN. |
