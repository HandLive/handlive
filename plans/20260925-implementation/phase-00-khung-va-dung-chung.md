...
| A0.1 [android] | Khung Gradle đa module, lint (ktlint, detekt), `core/protocol`, `core/crypto` qua S0.1, S0.2 | `android/` | `./gradlew test lint` xanh; vector xanh |
| A0.2 [android] | `core/transport`: Ktor WSS server với chứng chỉ tự ký trong Keystore, bắt tay phiên 0.7, capability 0.7.2, máy trạng thái 0.11 (chưa nối UI) | `android/core/transport` | Test JVM: hai phía bắt tay qua loopback, sai MAC bị từ chối đúng mã lỗi |
...
| R0.1 [relay] | Cargo workspace, `relay-server` khởi động với PostgreSQL + Redis (docker-compose cho dev), migration lược đồ 0.9.4, xác thực `device_id` tự chứng thực (C4), JWT ES256 | `relay/` | `cargo test` xanh; `docker compose up` chạy được; không đọc payload |
